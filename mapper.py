...
class InitMatrices(Initializer):
    """
...
class kernelized_mapper(Layer):
    ...
    def build(self, input_shape):
        ...
        self.Psi = self.add_weight(
            shape=(input_shape[2], input_shape[2]), 
            initializer=InitMatrices(diag=self.init_betas), 
            name='transformation', 
            trainable=True
        )

    def call(self, x):
        def mahalanobis(x, y, p):
            p = K.transpose(p)*p
            e = K.transpose(K.transpose(x)-y)
            return K.sqrt(K.sum(K.dot(e, p) * e, axis=-1))

        C = K.expand_dims(self.centers)
        C = K.expand_dims(C)

        H = mahalanobis(x, C, self.Psi + sys.float_info.epsilon)
        Mah  = K.exp(-self.betas * H)

        output = K.dot(Mah, self.kernel)
